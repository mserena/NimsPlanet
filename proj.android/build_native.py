...
def copy_files_exclude(src, dst, excl):
    ...
    for item in os.listdir(src):
        ...
        if not item.startswith('.') and not item.endswith('.gz') and os.path.isfile(path):
            shutil.copy(path, dst)
        if os.path.isdir(path) and not item.endswith('SoundiOS'):
            new_dst = os.path.join(dst, item)
            os.mkdir(new_dst)
            copy_files_exclude(path, new_dst, excl)
...
def copy_resources(app_android_root):
    ...
    if os.path.isdir(resources_dir):
        ...
        copy_files_exclude(resources_dir, assets_dir,"SoundiOS")
